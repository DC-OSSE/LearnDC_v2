...
overview_loc <- list.files("../data/school_overview")

overviews <- llply(overview_loc, function(sc) {
    sc_json <- jsonlite::fromJSON(paste0("../data/school_overview/", sc))
    with(sc_json, list(
        code=org_code,
        name=org_name,
...
profile_loc <- list.files("../data/school_report")

profiles <- llply(profile_loc, function(pf) {
    pf_json <- jsonlite::fromJSON(paste0("../data/school_report/", pf), simplifyDataFrame=FALSE)
    pf_json$code <- pf
