...
cas$lea_code[which(cas$lea_code!=1)] <- 0
cas$lea_name[which(cas$lea_code!=1)] <- 'Charter Sector LEA'
cas$lea_name <- toupper(cas$lea_name)
cas$lea_name[which(cas$lea_name=='DC PUBLIC SCHOOLS')] <- 'DISTRICT OF COLUMBIA PUBLIC SCHOOLS'
...
comp$lea_code[which(comp$lea_code!=1)] <- 0
comp$lea_name[which(comp$lea_code!=1)] <- 'Charter Sector LEA'
comp$lea_name <- toupper(comp$lea_name)
comp$lea_name[which(comp$lea_name %in% c('DCPS','DCPS         '))] <- 'DISTRICT OF COLUMBIA PUBLIC SCHOOLS'

science <- sqlQuery(dbrepcard, "SELECT * FROM [dbo].[assm_science]") 
science$lea_code[which(science$lea_code!=1)] <- 0
science$lea_name[which(science$lea_code!=1)] <- 'Charter Sector LEA'
science$lea_name <- toupper(science$lea_name)
...
sqlSave(dbrepcard_prod, lea_subgroups_df, tablename = "cas_sector_exhibit", append = FALSE, rownames=FALSE)
