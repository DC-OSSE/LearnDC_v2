...
state_hqt_df <- data.frame()
			...
			for(j in unique(.hqt_year$school_category)){
				...
				new_row <- cbind(.year,.school_category,.poverty_quartile,.total_classes,.hq_classes,.nhq_classes)
								
				state_hqt_df <- rbind(state_hqt_df, new_row)
			}
		}
	}
colnames(state_hqt_df) <- c("year","school_category","poverty_quartile","num_total_classes","num_hq_classes","num_nhq_classes")


sqlSave(dbrepcard_prod, state_hqt_df, tablename = "hqt_classes_state_exhibit", append = FALSE, rownames=FALSE)