...
for(a in unique(staff_degree$school_code)){
	...
	for(b in unique(degree_school$year)){
	 		tmp <- subset(degree_school,year==b)
			year <- b

	for(c in 0:length(unique(tmp$school_category))){
				if(c == 0){
					tmp_c <- tmp
					.school_category <- "All"
				} else{
					.school_category <- unique(tmp$school_category)[c]
					tmp_c <- subset(tmp, school_category == .school_category)
				}

		
		
			total <- nrow(tmp_c)
			associates <- sum(tmp_c$is_aa)
			bachelors <- sum(tmp_c$is_bachelors)
			masters <- sum(tmp_c$is_masters)
			phd <- sum(tmp_c$is_phd)
			none <- sum(tmp_c$is_none)

new_row <- c(year,lea_code,lea_name,school_code,school_name,.school_category,associates,bachelors,masters,phd,none,total)
school_degree <- rbind(school_degree,new_row)
			}
		}
	}
colnames(school_degree) <- c("year","lea_code","lea_name","school_code","school_name","school_category","num_associates","num_bachelors","num_masters","num_phd","num_none","num_total")

school_degree <- subset(school_degree,num_total==num_none & school_category!='All')
