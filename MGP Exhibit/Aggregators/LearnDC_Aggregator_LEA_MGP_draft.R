...
colnames(lea_subgroups_df) <- c("lea_code","lea_name","subgroup","year","group_fay_size","subject","mgp_1yr")

lea_subgroups_df$group_fay_size <- as.numeric(lea_subgroups_df$group_fay_size)
lea_subgroups_df$mgp_1yr <- round(as.numeric(lea_subgroups_df$mgp_1yr),2)
lea_subgroups_df$year <- as.numeric(lea_subgroups_df$year)

lea_subgroups_df_prior <- lea_subgroups_df
colnames(lea_subgroups_df_prior) <- paste0("prior_",colnames(lea_subgroups_df_prior))
lea_subgroups_df_prior$prior_year <- lea_subgroups_df$year + 1
lea_subgroups_df <- merge(lea_subgroups_df, lea_subgroups_df_prior, by.x = c("year","lea_code","lea_name","subgroup","subject"), by.y = c("prior_year","prior_lea_code","prior_lea_name","prior_subgroup","prior_subject"), all.x=TRUE)
lea_subgroups_df$mgp_2yr <- round((((lea_subgroups_df$mgp_1yr*lea_subgroups_df$group_fay_size)+(lea_subgroups_df$prior_mgp_1yr*lea_subgroups_df$prior_group_fay_size))/(lea_subgroups_df$group_fay_size+lea_subgroups_df$prior_group_fay_size)),2)

lea_subgroups_df$flag <- 0
lea_subgroups_df$flag[which(lea_subgroups_df$group_fay_size<=25|lea_subgroups_df$prior_group_fay_size<=25)] <- 1
lea_subgroups_df$mgp_2yr[which(lea_subgroups_df$flag==1)] <- NA

lea_subgroups_df <- select(lea_subgroups_df,year,lea_code,lea_name,subgroup,subject,group_fay_size,mgp_1yr,mgp_2yr)

sqlSave(dbworking, lea_subgroups_df, tablename = "mgp_lea_exhibit_draft", append = FALSE, rownames=FALSE)