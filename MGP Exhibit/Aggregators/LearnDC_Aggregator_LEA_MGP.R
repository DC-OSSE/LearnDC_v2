...
# lea_mgp <- sqlQuery(dbrepcard, "SELECT * FROM [dbo].[mgp_longitudinal]")
# lea_mgp <- subset(mgp, subgroup %notin% c("Not-LEP","Not-Economy","Not-SPED","Grade 4","Grade 5","Grade 6","Grade 7","Grade 8","Grade 10"))

# lea_mgp$school_year <- NA
# lea_mgp$school_year[which(mgp$year==2011)] <- '2010-2011'
# lea_mgp$school_year[which(mgp$year==2012)] <- '2011-2012'
# lea_mgp$school_year[which(mgp$year==2013)] <- '2012-2013'
# lea_mgp$school_year[which(mgp$year==2014)] <- '2013-2014'

# slims <- sqlQuery(dbedm,"select school_year,lea_code,lea_name,school_code,school_name from dbo.slims_extract")

# lea_mgp <- merge(lea_mgp,slims,by=c('school_year','school_code'),all.x=TRUE)
